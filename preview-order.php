    ...
    <div class="col l4 s12 m4 white-text card" id="black" style="background: #3A3A3A">
        ...
        <div class="card" style="background:transparent;padding:10px;border:1px solid #fff">
          <div class="card-content center white-text">
            ...
            <hr>
            <div class="row">
              <div class="col l4 m4"><p class="white-text">Products for </p></div>
              <div class="col l4 m4"><p class="white-text"><?php echo $_GET['product_id'];?></p></div>
            </div>
...
<?php
  ...
  $prd = $connect2db->prepare("SELECT p.id as prodID, p.user_id,p.product_id,p.quantity,p.order_id,p.total_amount,c.id,c.Coll_details,c.images FROM prd_order as p INNER JOIN collection AS c ON p.product_id=c.id WHERE user_id=? AND order_id=?");
  $prd->execute([$id,$prdID]);
  while($fprd = $prd->fetch()){
    $prodID = $fprd->prodID;
?>               <div class="row">
                    <div class="col l4 s4" style="padding-left:0px">
                      <img src="admin/collection/<?php echo $fprd->images;?>" style="width:130px;height:130px;margin-top:26px">
                    </div>
                    <div class="col l5 s5">
                      <div>
                      <p style="margin-bottom:0px;margin-top:30px"><?php echo $fprd->Coll_details;?></p></div>
                    ...
                    <div class="col l3 s3" style="padding:10px;"> 
                      <a href="profile-orders-detail.php?product_id=<?php echo $prodID;?>&order_id=<?php echo $prdID; ?>" class="left" style="color:#DAC08E;position:relative;top:65px;left:15%">Review</a></div>
                  </div>
          <?php }} ?>
