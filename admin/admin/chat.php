...
<?php 
	if (isset($_POST['id'])) {
		...
		$updateAgent = $connect2db->prepare("UPDATE chat SET receiver = ?, status = ? WHERE sender = ? AND m_date = ? ");
		$updateAgent->execute([$sender, 1, $receiver, $date]);
		echo($receiver);
		exit();
              ...
              <div class="table-responsive">
                ...
                <tbody>
                  <?php 
                    ...
                    $date = date('Y-m-d');
                    $getUser = $connect2db->prepare("SELECT chat.sender,chat.receiver,chat.status, chat.m_time,chat.m_date, users.role, users.fname,users.lname,users.country, users.id AS user,chat.id FROM chat JOIN users ON chat.sender = users.id WHERE chat.m_date = ? AND users.role != ? AND chat.status != 2 GROUP BY chat.sender ORDER by chat.id DESC ");
...
//                            echo $rc;
                          ...
                          ?></td>
                            ...
                            <td> 
                            	<?php 
                            		if ($row->status == 0) {?>
                          			<a data-toggle="modal" data-target="#defaultsizemodal" class="chat" id="<?php echo $row->user?>" data-name="<?php echo $cname?>" style="color:#AD976E;cursor:pointer;" > 
                            		Start Chat 
                            	</a>
                          		<?php } 
                              else if($row->status == 1 AND $row->receiver == $_SESSION['id']){?>
                                <a data-toggle="modal" data-target="#defaultsizemodal" class="chat" id="<?php echo $row->user?>" data-name="<?php echo $cname?>" style="color:#AD976E;cursor:pointer;" > 
                                Resume Chat 
                              </a>
