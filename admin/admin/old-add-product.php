			...
			$type = explode('.', $imageName);
	  		...
	  		if (in_array($type, $allow)) {
	  			if (move_uploaded_file($imageTemp, 'product/'.$imageName)) {
	  				$addProduct = $connect2db->prepare("INSERT INTO product (prd_name, price, quantity, highlight, description, status, featured, created, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?,?)");
					$query = $addProduct->execute([$pname, $price, $quantity, $highlight, $description, $status, $featured, $date,$imageName]);
					$query ?
