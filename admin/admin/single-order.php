...
if (isset($_POST['create'])) {
    ...
    file_put_contents($file_location, $file);
		...
		if ($mail->send()) {
			...
			$update = $connect2db->prepare("UPDATE tbl_order SET del_status = ?, file = ?, invoice = ?, del_date = ? WHERE user_id = ? AND order_id = ? ");
			$update->execute([$del_status, $file_name, $invoice, $del_date, $user_id, $order_id]);
			if ($update) {
				...
				$insertInvoice = $connect2db->prepare("INSERT INTO invoice (user_id, order_id, created, inv_type, invoice) VALUES (?,?,?,?,?)");
				$insertInvoice->execute([$user_id, $order_id, $date, 'product', $file_name]);
