...
class Sisow
{
	...
	public function DirectoryRequest(&$output, $select = false, $test = false) {
		if ($test === true) {
			...
			if ($select === true) {
				$output = "<select id=\"sisowbank\" class=\"browser-default\" name=\"issuerid\" style=\"color:background:transparent;\">";
		...
		if ($select === true) {
			$output = "<select id=\"sisowbank\" class=\"browser-default\" name=\"issuerid\" style=\"background:transparent;\">";
		}
		else {
			$output = array();
		}
