<?php include 'includes/connection.php';
  ...
  if(isset($_POST['register'])){
	if (isset($_POST['fname'])) {
		...
		$stmt->execute([$email]);
		if ($stmt->rowcount()>0) {
			echo "<script>alert('User already Available Please Use the Login');window.location='login.php';</script>";
		} else{
			...
			$query ?
			print "<script>alert('Registration Successful, You can now login');window.location='login.php';</script>":
