...
//$pdfDoc = '<div style="background:#3A3A3A;color:#fff;padding:13px;"><div><img alt="LOGO" src=cid:logo></div>';

$pdfDoc ='<h4 style="padding-top:24px;font-style: normal;font-weight: 500;font-size: 22px;line-height: 33px;">Hello '.$fname." ".$lname.' </h4>';

$pdfDoc .= '<p style="padding-top:24px;font-style:normal;font-weight:300;font-size:20px;"> Thank you for your order from EchteZalm. Your packaged has been processed and ready to be delivered to you. Your package would arrive between 1 - 2 days from now.
Your order confirmation is below, thanks again for Patronizing us.</p>';

$pdfDoc .='<h4 style="padding-top:24px;font-style:normal;font-weight:500;font-size:22px;line-height:33px;">Your Order #'.$order_id.' <small>('.$date.')</small></h4>';

$pdfDoc.='<table class="table" style="width:80%;border:px solid #000;vertical-align:top">
	<tbody>
		<tr>
			<td>
                <table style="width:100%;border:1px solid #000;">
                    <thead style="background:#0f0f0f;"><tr><th>Shipping Address</th></tr></thead>
            ...
            <td valign="top">
                <table style="width:100%;border:1px solid #000;">
                    <thead style="background:#0f0f0f;width:100%"><tr><th>Payment Information</th></tr></thead>
                    <tbody style="background:#3A3A3A;">
...
$pdfDoc.='<table class="table table-striped" style="width:80%;border:1px solid #000;" >
            <thead style="background:#0f0f0f;"><tr><th colspan="2"><center>Item Ordered</center></th></tr></thead>
                <tbody>
...
//		file_put_contents($file_location, $file);
        ...
        $mail->AddEmbeddedImage("images/logo-white.png", 'logo-white');
		$mail->Body = '<div style="background:#3A3A3A;color:#fff;padding:13px;"><div><img alt="LOGO" src="cid:logo-white"></div>'.$pdfDoc;

		if ($mail->send()) {
			$del_status = 'Processing';
			$invoice = 1;
			$del_date = date('Y-m-d H:i:s', strtotime("+48 hours"));
			$update = $connect2db->prepare("UPDATE tbl_order SET del_status = ?, invoice = ?, del_date = ? WHERE user_id = ? AND order_id = ? ");
			$update->execute([$del_status, $invoice,  $del_date, $user_id, $order_id]);
			if ($update) {
				...
				$insertInvoice = $connect2db->prepare("INSERT INTO invoice (user_id, order_id, created, inv_type) VALUES (?,?,?,?)");
				$insertInvoice->execute([$user_id, $order_id, $date, 'product']);
