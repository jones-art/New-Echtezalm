      ...
      <div class="card" style="background:transparent;">
        <div class="card-content white-text">
          <div class="card-title left-align" style="margin-bottom:30px">
          	<a href="product_page_description.php?id=<?php echo $encid;?>" class="webShpBtn center" > Description</a>
          	<a href="product_page_review.php?id=<?php echo $encid;?>"class="webShpBtn center" style="background-color:transparent;" >Reviews</a>
          </div>
            <!--   Review Start From Here-->
             <p class="left-align" style="align-content:justify">
          	<?php 
                $getReview = $connect2db->prepare("SELECT r.rating, r.title, r.message,r.created AS reviewed,r.visibility, u.fname,u.lname FROM review AS r INNER JOIN users AS u ON r.user_id = u.id WHERE prd_id = ? AND r.status = ? AND r.visibility = ?");
                $getReview->execute([$data, 1, 1]);
                if($getReview->rowcount()>0){
                    while($review = $getReview->fetch()){?>
                         <div class="card" style="background:transparent;">
                            <div class="card-content white-text">
                              ...
                              <span class="card-title left-align" style="margin-top:20px;margin-bottom:20px"><?php echo $review->title; ?></span>
                              <p class="left-align">
                                <?php echo $review->message; ?>
                                </p>
                                <p class="left-align" style="margin-top:20px;margin-bottom:23px"> 
                                <?php echo $review->reviewed; ?> door <?php echo $review->fname." ".$review->lname; ?></p>
                 ...
                 <?php   }
                    ...
                    <div>Product Not Yet Reviewed</div>
               <?php }
            ?>
			</p>
            <!-- Review Ends Here-->
		...
		let items = [];
			...
			if (typeof(Storage !== 'undefined')) {
				...
				if (JSON.parse(localStorage.getItem('items')) === null) {
					...
					$('#localStorage').html('Added to Cart');
                    $('#localStorage').attr('disabled', true);
				}else{
