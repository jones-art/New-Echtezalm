...
<?php
  $p = $connect2db->query("SELECT count(id) as id FROM tbl_order WHERE user_id=$id AND MONTH(ord_date)=MONTH(CURRENT_DATE())");
          ...
          <div class="card-content center white-text">
            ...
            <div class="row center" style="margin-top:25px;padding-left:0px;margin-bottom:0px">
              <div class="col l11 m11">
                  <div class="row">
<?php
  $prd = $connect2db->prepare("SELECT * FROM tbl_order WHERE user_id=?");
  $prd->execute([$id]);
  while($fprd = $prd->fetch()){
?>                    <div class="col l12 s12" style="border:1px solid #fff;margin-left: 10px;margin-bottom: 10px">
                      <table style="line-height:0px">
                        <tr>
                          <th> Order ID:</th>
                          <td> <?php echo $fprd->order_id; ?></td>
                          <td rowspan="4"> <a href="preview-order.php?product_id=<?php echo $fprd->order_id;?>" class="left" style="color:#DAC08E;position:relative;left:15%">Zie de details</a></td>
                        </tr>
                        <tr>
                          <th> Placed on:</th>
                          <td> <?php echo $fprd->ord_date; ?></td>
                        </tr>
                        <tr>
                          <th> Total items:</th>
                          <td> <?php echo $fprd->order_id; ?></td>
                        ...
                        <tr>
                          <th> Status:</th>
                          <td style="color:<?php if($fprd->del_status=='Pending'){echo '#c1c1c1;font-weight:300';}elseif($fprd->del_status=='Canceled'){echo '#F64F4F';}else{echo '#7CF094';}?>"> <?php echo $fprd->del_status; ?></td>
                        </tr>
                      </table>
                      <?php 
