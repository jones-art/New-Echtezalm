...
<?php 
  ...
  if(isset($_POST['editCollectionPage'])){
    ...
    // First Image Slidder
    $path = 'images/slidder/';  
    $targetPathImage = $path . basename($_FILES['webBanner']['name']); 

    $file_name = $_FILES['webBanner']['name'];
    $file_tmp = $_FILES['webBanner']['tmp_name'];
    $file_size = $_FILES['webBanner']['size'];
    $file_error = $_FILES['webBanner']['error'];
    $file_type = $_FILES['webBanner']['type'];
    ...
    if ($ptitle=="" OR $ptitle == " " OR empty($ptitle)) {
      ...
      $sql = $connect2db->prepare("SELECT page_name, page_title FROM pages WHERE page_name='Collection'");
      $sql->execute();
      $row = $sql->rowcount();
      if($row > 0){
        ...
        if($update->execute()){
           if(move_uploaded_file($_FILES['webBanner']['tmp_name'], $targetPathImage)){ 
        ...
        <form class="container" id="loginForm" enctype="multipart/form-data" method="POST">
          <div class="col-sm-12">
            <div class="alert text-right" role="alert">
              <button type="submit" class="btn btn-primary" name="editCollectionPage"> Save</button>
              <hr>
            </div>
                ...
                <div id="user-role-section" style="border: 1px solid #626567 ">
                  <div class="row">
                    <div class="col-sm-3 py-3 my-3">
                      ...
                      <div class="col-sm-10">
                        <input type="file"  name="webBanner" id="file" style="display:none;position:relative;top:20px" accept=".jpg, .png, .jpeg" onchange="loadFile(event)">
                      </div>
                    </div>
                  </div>
