	...
	if(isset($_POST['submit'])){
		...
		// echo "<script> alert($month); </script>";
			...
			if($sql->rowcount() > 0){
				...
				$getData = $connect2db->prepare("INSERT INTO subscriber(order_id,status,payment_status,user_id,sub_date,plan,month,duration,sub_total) VALUES(?,?,?,?,?,?,?,?,?)");
				$getData->execute([$myRandom,'Pending','Pending',$id,$date,$plan,$month,$month,$sub_total]);
				if($getData){
			...
			<div class="col l6 s12 m6">
				<div class="card" style="background:#3A3A3A;padding-left:20px;padding-top:20px;padding-bottom:23px">
					<div class="card-body">
						...
						<p class="sub-title"><?php echo($records->Coll_details)?> wine - 5, 15 or 30 pieces</p>
						<a href="<?php if (isset($_SESSION['email'])) {
                      echo 'collection_page_description.php?id=1';}else{echo 'login.php';}?>" style="margin-top:30px;margin-bottom:15px;color:#AD976E;font-weight:500">Change</a>
					</div>
				</div>
				<form method="POST">
